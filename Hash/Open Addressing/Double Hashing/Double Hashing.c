...
void insert(struct Student rec, struct Record table[])
{
    int i, location, h, h2, r = 17;
    int key = rec.studentId;
    h = hash(key);      // primary hash function
    h2 = r - (key % r); // secondary hash function. r is smaller than TSIZE and is prime
