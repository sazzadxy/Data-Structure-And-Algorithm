...
void deleteFromEdgeLists(char s1[]);
void deleteFromVertexList(char s1[]);
...
void insertEdge(char s1[],char s2[])
{
    ...
    if(strcmp(s1,s2) == 0)
    {
        printf("Invalid Edge: Start and end vertices are same\n\n");
        return;
    }
    ...
    if(u == NULL)
    {
        printf("Start vertex not present, first insert vertex\n\n");
        return;
    }
    if(v == NULL)
    {
        printf("End vertex not present, first insert vertex\n\n");
        return;
    }

    // Allocate new edge node with end vertex v
