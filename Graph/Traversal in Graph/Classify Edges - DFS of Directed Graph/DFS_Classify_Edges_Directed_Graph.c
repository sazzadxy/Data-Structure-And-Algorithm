...
void dfs(int v)
{
    ...
    for(i = 0; i < n; i++)
    {
        if(adj[v][i] != 0)
        {
            ...
            else if(vertexList[i].state == VISITED)  // Edge to ancestor vertex
                printf("Back Edge - (%s, %s)\n", vertexList[v].name, vertexList[i].name);
            else if(vertexList[v].discoveryTime < vertexList[i].discoveryTime)  // Edge to non son descendant
                printf("Forward Edge - (%s, %s)\n", vertexList[v].name, vertexList[i].name);
            else  // Edge to different vertex of tree graph or spanning tree not ancestor or descendant
